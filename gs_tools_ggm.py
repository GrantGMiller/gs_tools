...
class event():
    def __init__(self, objs, eventNames, nonDestructive=False):
        # print('__init__(objs={}, eventNames={})'.format(objs, eventNames))
        
        self.nonDestructive = nonDestructive #If this option is set to True, then the functionality will be added without affecting the original functionality.
        ...
        for obj in self.objs:
            for eventName in self.eventNames:
                ...
                if self.nonDestructive:
                    ...
                    OldHandler = getattr(obj, eventName)
                    if OldHandler == None:
                        setattr(obj, eventName, func)
                    else:
                        def GetNewHandler():
                            def NewHandler(button, state):
                                ...
                                func(button, state)
                            return NewHandler
                        setattr(obj, eventName, GetNewHandler())
                        
            if hasattr(obj, 'CheckEventHandlers'):
                obj.CheckEventHandlers()
        return func
        
class event_nonDestructive(event):
    def __init__(self, *args, **kwargs):
        kwargs['nonDestructive'] = True
        super().__init__(*args, **kwargs)
...
def HandleConnection(interface):
    ...
    if (isinstance(interface, extronlib.device.UIDevice) or
...
class PollingEngine():
    ...
    def __DoAQuery(self):
        ...
        try:
            ...
            #print('PollingEngine Error:\ninterface={}\nCommand={}\nQualifier={}\nException={}'.format(Interface, Command,Qualifier, e))
            pass
                ...
                if d['command'] == command:
                    if d['qualifier'] == qualifier:
                        ...
                        if 'value' in d:
                            ...
                            if d['value'] == None:  # if the user didnt provide a 'value' parameter, then set the text/state for any value
                                if 'match' in d:
                                    if d['match'] == True:
                                        if 'text' in d:
                                            obj.SetText(value)
